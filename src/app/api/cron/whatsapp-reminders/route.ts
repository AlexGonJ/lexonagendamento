...
import { runAppointmentRemindersJob } from "@/actions/whatsapp";
...
export async function GET(req: Request) {
  try {
    ...
    // Executa a automação de lembretes para todos os estabelecimentos com a opção ativa
    ...
    for (const tenant of activeTenants) {
      const res = await runAppointmentRemindersJob(tenant.id, cronSecret);
