import { getBookings, updateBookingStatus, deleteBooking, rescheduleBooking } from "@/actions/booking";
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <a
          ...
          className="inline-flex items-center justify-center rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-gray-700"
        >
          Exportar CSV
        </a>
      </div>
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {booking.status === "CONFIRMED" && (
                            <>
                              <form action={async () => { "use server"; await updateBookingStatus(booking.id, "COMPLETED"); }}>
                                <button type="submit" className="px-2.5 py-1.5 bg-violet-600 hover:bg-violet-700 text-white font-medium text-xs rounded-lg transition-colors cursor-pointer">
                                  Concluir
                                </button>
                              </form>
                              <form action={async () => { "use server"; await updateBookingStatus(booking.id, "NO_SHOW"); }}>
                                <button type="submit" className="px-2.5 py-1.5 bg-gray-600 hover:bg-gray-700 text-white font-medium text-xs rounded-lg transition-colors cursor-pointer">
                                  Faltou
                                </button>
                              </form>
                            </>
                          ...
                          {["PENDING", "CONFIRMED"].includes(booking.status) && (
                            ...
                            >
                              <input name="date" type="date" defaultValue={booking.date.toISOString().slice(0, 10)} className="w-28 rounded border border-gray-300 px-1 py-1 text-xs" aria-label="Nova data" />
                              <input name="time" type="time" defaultValue={booking.date.toISOString().slice(11, 16)} className="w-20 rounded border border-gray-300 px-1 py-1 text-xs" aria-label="Novo horário" />
                              <button type="submit" className="px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs rounded-lg transition-colors cursor-pointer">
                                Reagendar
                              </button>
                            </form>
                          )}
